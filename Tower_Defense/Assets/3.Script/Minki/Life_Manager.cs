...
public class Life_Manager : NetworkBehaviour
{
    ...
    public void Life_Set(int player_num, int monster_num)
    {
        ...
        if (isServer)
        {
            Kill_Log.instance.Adding_Message($"<color=E34E4E> {player_name[monster_num-1]} </color> 님이 <color=FFFFFF> {player_name[player_num-1]} </color>님의 <color=FFFFFF> Life 1 </color>을 획득합니다.");
        }
        }
        Die(player_num);
    }
